...
### Added
- **CSI driver control plane** (`csi_driver.py`) — ankavm storage as k8s
  PersistentVolumes, exposed as `/api/v2/csi/{info,volumes}`.
- **KubeVirt bridge** (`kubevirt_bridge.py`) — register clusters, lower
  VirtualMachineInstance specs to ankavm VM configs.
- **GitOps manager** (`gitops_manager.py`) — register Git repos with
  ArgoCD/Flux directory layout; manual sync endpoint.
- **Firecracker microVM runtime** (`firecracker_runtime.py`) — second-tier
  VM type with ~125ms boot + ~5MB overhead.
- **OAuth2 provider presets** (`oauth2_presets.py`) — Keycloak,
  Authentik, Okta, Entra ID, Google, GitLab one-click discovery URL.
- **Audit-log retention policy** (`audit_retention.py`) — configurable
  age + size cap, JSONL trim pass under `/api/v2/audit/retention`.
- **CycloneDX SBOM generator** (`sbom_generator.py`) — auto-generated by
  `.github/workflows/i18n-and-sbom.yml`.
- **PWA offline manifest** (`pwa_offline.py`) — service-worker cache list
  + API fallback config.
- **Sixth language: French (FR)** — `LANGS.fr` block + 2214 FR
  PAGE_STRINGS entries.
- **i18n CI gate** (`.github/workflows/i18n-and-sbom.yml`) — fails the
  build if untranslated TR strings appear in HTML or JS.
- **i18n pipeline** (`scripts/i18n_pipeline.py`, `Makefile`,
  `.git-hooks/pre-commit`) — `make i18n` runs extract → html_scan →
  js_scan → augment → inject. The pre-commit hook reruns it whenever
...
- Version bumped 2.7.1 → 2.7.2 in app.py.
...
- Privacy Policy page (ankavm.local/privacy) — GDPR scope.
