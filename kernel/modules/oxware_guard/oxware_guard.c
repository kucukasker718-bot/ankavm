...
/*
 * oxware_guard.ko — OXware Hypervisor Memory Guard Module
 * Detects unauthorized writes to /opt/oxware runtime memory mappings.
 * Uses userfaultfd + mprotect + kretprobe on mmap syscall.
 * Target: Ubuntu 22.04 / Debian 12 (kernel 5.15 – 6.8)
 */
...
MODULE_AUTHOR("OXware");
MODULE_DESCRIPTION("OXware Memory Guard v" OXGUARD_VERSION);
MODULE_VERSION(OXGUARD_VERSION);

/* Alert counter — userspace polls /dev/oxware_guard */
...
static void ox_alert(const char *msg)
{
    ...
    pr_warn("oxware_guard: ALERT pid=%d comm=%s — %s\n",
            current->pid, current->comm, msg);
}

/* kretprobe: mmap — detect PROT_WRITE|PROT_EXEC mappings (W^X violation) */
static int handler_mmap_ret(struct kretprobe_instance *ri, struct pt_regs *regs)
{
    /* regs->ax = return value (new mmap addr) — check calling process */
    if (strncmp(current->comm, "python3", 7) == 0 ||
         ...
         * but we detect if oxware process makes anomalous mappings */
...
static int __init oxware_guard_init(void)
{
    int ret;
    ret = misc_register(&oxg_miscdev);
    if (ret) { pr_err("oxware_guard: misc_register: %d\n", ret); return ret; }

    ret = register_kretprobe(&krp_mmap);
    if (ret < 0) pr_warn("oxware_guard: mmap probe: %d\n", ret);

    ret = register_kprobe(&kp_ptrace);
    if (ret < 0) pr_warn("oxware_guard: ptrace probe: %d\n", ret);

    pr_info("oxware_guard: v%s loaded\n", OXGUARD_VERSION);
    return 0;
}

static void __exit oxware_guard_exit(void)
{
    unregister_kretprobe(&krp_mmap);
    unregister_kprobe(&kp_ptrace);
    misc_deregister(&oxg_miscdev);
    pr_info("oxware_guard: unloaded\n");
}

module_init(oxware_guard_init);
module_exit(oxware_guard_exit);
