...
/*
 * oxware_audit.ko — OXware Hypervisor Audit Kernel Module
...
MODULE_DESCRIPTION("OXware Hypervisor Audit Module v" OXWARE_AUDIT_VERSION);
...
static struct oxware_event  ox_ring[OXWARE_RING_SIZE];
...
static void ox_emit(u32 type, const char *msg)
{
    ...
    audit_log(audit_context(), GFP_ATOMIC, AUDIT_KERN_MODULE,
              "oxware: type=%u pid=%d comm=%s msg=%s",
              type, current->pid, ev->comm, ev->msg);
}

/* ── kprobe: kvm_vm_ioctl (VM lifecycle) ──────────────────────────────── */
...
/* ── kprobe: do_execve (detect libvirt/qemu exec) ─────────────────────── */
...
/* ── /dev/oxware_audit — userspace read interface ──────────────────────── */
...
/* ── Module init / exit ────────────────────────────────────────────────── */
static int __init oxware_audit_init(void)
{
    int ret;

    ret = misc_register(&ox_miscdev);
    if (ret) {
        pr_err("oxware_audit: misc_register failed: %d\n", ret);
        return ret;
    }

    ret = register_kprobe(&kp_kvm_ioctl);
    if (ret < 0)
        pr_warn("oxware_audit: kvm_vm_ioctl probe failed: %d (KVM not loaded?)\n", ret);

    ret = register_kprobe(&kp_execve);
    if (ret < 0)
        pr_warn("oxware_audit: do_execve probe failed: %d\n", ret);

    pr_info("oxware_audit: v%s loaded — /dev/oxware_audit ready\n", OXWARE_AUDIT_VERSION);
    ox_emit(OXEVENT_VM_CREATE, "oxware_audit_init");
    return 0;
}

static void __exit oxware_audit_exit(void)
{
    unregister_kprobe(&kp_kvm_ioctl);
    unregister_kprobe(&kp_execve);
    misc_deregister(&ox_miscdev);
    pr_info("oxware_audit: unloaded\n");
}

module_init(oxware_audit_init);
module_exit(oxware_audit_exit);
